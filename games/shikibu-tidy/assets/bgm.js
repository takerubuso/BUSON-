'use strict';
/* BGM エンジン（しきぶのおかたづけ 修正 7。第2弾 ref/cafe-game/bgm.js をそのまま移植。中身の動きは同じ）— tools/build_bgm.py が作った MP3
   （assets/bgm_data.js に base64 で埋め込み）を Web Audio で鳴らす。
   title・main はループ（BufferSource の loop。loopStart/loopEnd はビルドが拍と相関で決めた継ぎ目。継ぎ目の 30ms の等パワーのつなぎは
   ファイルに焼き込み済みで、飛ぶ所の前後は同じ音）。ジングルは 3 本: clear（クリアのカード）・fail（失敗）はステージ中の曲を DUCK_DB 下げて重ね、
   終わったら戻す。reveal（部屋のお披露目）はステージ中の曲を止めてから鳴らす（B.stop。つぎのステージでまた流れる）。
   経路: 曲ごとの gain → duck（ジングルの間だけステージ中の曲を下げる）→ out（BGM の ON/OFF）→ 出力。効果音のローパスとコンプレッサーは通さない
   ...
   ゲーム（index.html の Sound）と自動テストが同じものを使う。外部通信はしない。 */
function createBgmEngine() {
  const B = {
    ...
    XFADE: 0.4, DUCK_DB: -10, DUCK_IN: 0.15, DUCK_OUT: 0.6, MUTE_FADE: 0.15, STEPS: 8,
  };
  ...
  // data = BGM_DATA（{ tracks: { title: { b64, loopStart, loopEnd, sec }, main: {...}, clear: { b64, sec }, reveal: {...}, fail: {...} } }）。
  ...
  // ループの曲を止める（fade 秒で）。want も消すので、復号が後から済んでも鳴らない
  B.stop = function (o) {
    o = o || {}; B.want = null;
    const ctx = B.ctx; if (!ctx || !B.cur) return null;
    const t = o.when != null ? o.when : ctx.currentTime, fade = o.fade != null ? o.fade : B.XFADE, old = B.cur;
    ramp(old.gain.gain, old.gain.gain.value, 0, t, fade); try { old.src.stop(t + fade + 0.05); } catch (e) { /* 止め済み */ }
    B.cur = null; B.log.push({ name: null, from: old.name, t: r3(t), fade });
    return true;
  };
  // ジングル（name = 'clear'・'reveal'・'fail'）を 1 回。duck = true なら鳴っている間だけループの曲（duck）を DUCK_IN 秒で DUCK_DB 下げ、終わったら DUCK_OUT 秒で戻す。
  // ジングルは duck を通らない（BGM の ON/OFF には従う）
  B.jingle = function (name, o) {
    o = o || {};
    const ctx = B.ctx, buf = B.buffers[name];
    if (!ctx || !buf) return null;
    const t = o.when != null ? o.when : ctx.currentTime, sec = B.info[name].sec;
    ...
    B.jingleAt = t; B.jingleEnd = t + sec; B.jingleName = name;
    B.log.push({ name, jingle: true, t: r3(t), sec: r3(sec) });
    return { name, when: t, sec };
  };
  ...
  B.state = function () {
    return { want: B.want, cur: B.cur ? B.cur.name : null, on: B.on, decoded: B.decoded, failed: B.failed, starts: B.starts,
      ...
      jingleAt: r3(B.jingleAt), jingleEnd: r3(B.jingleEnd), jingleName: B.jingleName || null, log: B.log.slice(-12), info: JSON.parse(JSON.stringify(B.info)) };
  };
  return B;
}
