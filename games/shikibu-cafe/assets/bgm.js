'use strict';
/* BGM エンジン（合体カフェ 修正 3。第1弾 ref/sort-game/bgm/bgm.js を移植）— tools/build_bgm.py が作った MP3（assets/bgm_data.js に base64 で埋め込み）を Web Audio で鳴らす。
   title・main はループ（BufferSource の loop。loopStart/loopEnd はビルドが拍と相関で決めた継ぎ目。継ぎ目の 30ms の等パワーのつなぎは
   ファイルに焼き込み済みで、飛ぶ所の前後は同じ音）。ジングルは 2 本: legend（伝説のお菓子。営業中の曲を DUCK_DB 下げて重ね、終わったら戻す）、
   close（閉店。営業中の曲を止めてから鳴らし、あとは静か。B.stop）。
   経路: 曲ごとの gain → duck（伝説のジングルの間だけ営業中の曲を下げる）→ out（BGM の ON/OFF）→ 出力。効果音のローパスとコンプレッサーは通さない
...
function createBgmEngine() {
  ...
  // data = BGM_DATA（{ tracks: { title: { b64, loopStart, loopEnd, sec }, main: {...}, close: { b64, sec }, legend: { b64, sec } } }）。
  ...
  // ジングル（name = 'legend' か 'close'）を 1 回。duck = true なら鳴っている間だけループの曲（duck）を DUCK_IN 秒で DUCK_DB 下げ、終わったら DUCK_OUT 秒で戻す。
